/**
 * CityMotion — Settings: Renderizadores de UI (HTML puro)
 */
...
export function statusBadge(testResults, section) {
  const r = testResults[section];
  if (!r) return '';
...
export function renderOperationsTab(state) {
...
export function renderReportDesignTab(t, saving, reportTab) {
  const subTabs = [
  ...
  return `
    <div class="space-y-6">
      <div class="flex items-center gap-4 mb-2">
        ...
        ${saving ? '<span class="text-[10px] text-primary animate-pulse">Salvando...</span>' : ''}
      </div>
      ...
      <div class="flex gap-1 flex-wrap p-1 bg-zinc-900/60 border border-zinc-800/50 rounded-lg">
        ${subTabs.map(st => `
          <button onclick="document.getElementById('settingsOverlay').dispatchEvent(new CustomEvent('setRepTab', {detail: '${st.id}'}))"
            class="text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-md transition-all ${reportTab === st.id ? 'bg-primary/20 text-primary border border-primary/30' : 'text-zinc-500 hover:text-zinc-300'}">
      ...
      ${reportTab === 'header' ? renderRepHeader(t) : ''}
      ${reportTab === 'colors' ? renderRepColors(t) : ''}
      ${reportTab === 'footer' ? renderRepFooter(t) : ''}
      ${reportTab === 'sector' ? renderRepSector(t) : ''}

      <div class="flex justify-end gap-3 pt-4 border-t border-zinc-800/30">
        <button onclick="document.getElementById('settingsOverlay').dispatchEvent(new CustomEvent('resetRepTemplate'))" class="btn btn-ghost btn-sm text-[10px]">Resetar Padrão</button>
        <button onclick="document.getElementById('settingsOverlay').dispatchEvent(new CustomEvent('saveRepTemplate'))" class="btn btn-primary btn-sm text-[10px] px-8" ${saving ? 'disabled' : ''}>
          ${saving ? '<svg class="w-3 h-3 mr-2 inline animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>' : '<svg class="w-3 h-3 mr-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"/></svg>'}
          Salvar Template
        </button>
      </div>
    </div>`;
...
export function renderRepHeader(t) {
...
export function renderRepColors(t) {
...
export function renderRepFooter(t) {
  return `
    <div class="glass-card rounded-xl p-6 border-zinc-700/50 bg-zinc-900/40">
...
export function renderRepSector(t) {
  return `
    <div class="glass-card rounded-xl p-6 border-zinc-700/50 bg-zinc-900/40">
...
export function renderInfraTabs(cfg, state, testResults) {
  return `<div class="space-y-6">
    ...
    <div class="space-y-4">
      <div><label class="text-[10px] font-bold uppercase tracking-widest text-zinc-500 block mb-2">Tipo</label>
        <select id="infraDbType" class="w-full px-3 py-2 bg-black/40 border border-zinc-700/50 rounded-lg text-sm focus:outline-none focus:border-primary/50">
          ${dbTypes.map(db => `<option value="${db.value}" ${state.dbType===db.value?'selected':''}>${db.label} — ${db.desc}</option>`).join('')}</select></div>
...
function renderSmtpTab(state, testResults) {
  return `<div class="glass-card rounded-xl p-6 border-zinc-700/50 bg-zinc-900/40">
    <div class="mb-4 border-b border-zinc-800/30 pb-4"><h3 class="text-xs font-bold uppercase tracking-widest flex items-center gap-2"><svg class="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>SMTP ${statusBadge(testResults, 'smtp')}</h3></div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
...
function renderServerTab(state, cfg) {
